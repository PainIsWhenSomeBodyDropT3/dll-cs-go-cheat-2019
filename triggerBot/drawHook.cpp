...
void drawHook::Patch(BYTE* dst, BYTE* src, unsigned int size) {
	DWORD oProc;
...
// hook function
bool drawHook::Hook(char* src, char* dst, int len) {
...
char* drawHook::TrampHook(char* src, char* dst, unsigned int len) {
	if (len < 5) return 0;
